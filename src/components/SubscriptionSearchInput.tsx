...
import { type Subscription } from "@/server/db/schema"; // Import Subscription type
...
const SubscriptionSearchInput: React.FC = () => {
  ...
  const debouncedQuery = useDebounce(query, 300); // Debounce with a 300ms delay
  ...
  const handleInputFocus = () => {
    // Show results if there are any when the input is focused
  ...
  // Animation variants for individual result items
  ...
  return (
    <div className="relative w-full"> {/* Remove max-w and mx-auto */}
      <div className="relative flex items-center">
        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-500" /> {/* Search icon */}
        <Input
          type="text"
          ...
          className="w-full px-10 py-3  border border-gray-700 rounded-md focus:outline-none focus:ring-0 focus:border-blue-500" // Dark theme styling
        />
        {loading && (
      ...
      <AnimatePresence>
        {showResults && (searchResults.length > 0 || (debouncedQuery && !loading && searchResults.length === 0)) && (
          <motion.div
            className="absolute z-10 w-full bg-white border border-gray-700 rounded-md shadow-lg mt-2 max-h-60 overflow-y-auto" // Dark theme styling
            ...
            {searchResults.length > 0 ? (
              <ul>
                {searchResults.map((subscription) => (
                  ...
                  >
                    {/* Assuming your subscription details page is at /dashboard/subscriptions/[id] */}
                    <Link href={`/dashboard/subscriptions/${subscription.id}`} className="block px-4 py-2 hover:bg-gray-100 cursor-pointer flex items-center w-full"> {/* Dark theme styling */}
              ...
              <div className="px-4 py-2 text-center text-sm text-gray-400"> {/* Dark theme styling */}
